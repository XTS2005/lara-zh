    ...
    var body: some View {
        List {
            ...
            Section(
                ...
                footer: Text("选择要禁用的守护进程。ScreenTimeAgent 和 UsageTrackingAgent 是完全禁用屏幕使用时间的最低要求。")
            ) {
            ...
            Section(
                ...
                footer: Text("终止选定的守护进程，移除屏幕使用时间偏好设置，并在 launchd 的 disabled.plist 中将其标记为禁用。需要重启才能使更改生效。")
            ) {
                Button {
                    applyDisable()
                } label: {
